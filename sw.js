// 빵을그리다 통합 재고관리 오프라인 캐시 — index.html·finished.html(완제품·생지)은 네트워크 우선(브라우저 HTTP 캐시 우회 → 최신 배포 즉시 반영), 실패 시 캐시
const CACHE = 'wonryo-v055';
...
self.addEventListener('fetch', e => {
  ...
  const isPage = e.request.mode === 'navigate' || /index\.html$|finished\.html$|\/$/.test(new URL(e.request.url).pathname);
